...
def generate():
    ...
    spacers = Box(gamegrid, height=100, width=10, grid=[0, 3])
    ...
    for i in range(4):
        ...
        spacers = Box(gamegrid, height=100, width=10, grid=[0, i + 5])
    ...
    button = PushButton(gamegrid, text="<-- retour", grid=[0, 11], command=retouraumenu, width=10)
...
def musiquepage():
    ...
    buttonquitter = PushButton(griddemusique, text="<-- retour", grid=[0, r + 3], command=quittermusique, width=10)
...
def logpage():
    ...
    if (logreponse == []):
        spacer = Box(griddelog, height=70, width=10, grid=[0, 1])
        no = Text(griddelog, text="Aucune donnée il faut jouer pour voir apparaitre les logs", size=15, font="Arial", color="white", grid=[0, 2])
    else:

        for j in logreponse:
            log = Text(griddelog, text=j, size=10, font="Arial", color="white", grid=[0, logreponse.index(j) +1 ])
    spacer = Box(griddelog, height=100, width=10, grid=[0, 11])
    buttonquitter = PushButton(griddelog, text="<-- retour", grid=[0, 12], command=quitterlog, width=10)
